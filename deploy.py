...
col1.title('The Model in ACTION!')
col1.header('Enter the feature values')
features['state'] = col1.selectbox("The customer's state", list(df['state'].unique()))
features['total_day_minutes'] = col1.slider("Total time spoken during the day", 0.0, 400.0, 161.6)
features['number_customer_service_calls'] = col1.slider("Total number of customer service calls", 0, 10, 1)
features['total_eve_minutes'] = col1.slider("Total time spoken during the evening", 0.0, 359.0, 195.5)

# original_title = '<p style="font-family:Courier; color:Blue; font-size: 20px;">'
col1.header('More Inputs')
with col1.expander("Advanced Features:"):
...
if col1.button('Predict'):
    ...
    else: col1.success("This customer is under the risk of churning."
...
col2.title('')
col2.header(" ")
col2.header('Probability Density: Features vs Churn'
           )
option = col2.selectbox( 'Input Features', tuple(df.columns))

# with st.sidebar:
if option:
        col2.plotly_chart(hist(option) ,use_container_width = True)
